...
def elbow_method(X, Ks, times, initial_random_state):
    ...
    plot_curves_with_legend(Ks, Ws, legends = list(range(times)), X_label = "K", Y_label = "W")
...
def kohonen_matrix_predictions(movies_df, only_genres_df):
    ...
    unsupervised_classifier = UnsupervisedClassifier("kohonen", K=5, max_iter=100)
    ...
    kohonen_predictions = unsupervised_classifier.model.predict(
        movies_df.values)
    plot_kohonen_matrix_predictions(unsupervised_classifier._model, y, kohonen_predictions, classes)
