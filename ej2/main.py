...
def pca_plot_hierarchical(X, model, title):
    pca, pca_data = apply_pca_to_data(data=X, n_components=2)
    ...
    for k in range(3, 10):
        df_clusters = pd.DataFrame()
        for i, cluster in enumerate(model.get_kth_cluster(k)):
            df_points = pd.DataFrame(
                cluster.points, columns=pca_data_df.columns)
            df_points["cluster"] = i
            df_merged = pd.merge(pca_data_df, df_points)
            df_clusters = pd.concat([df_clusters, df_merged])
...
if __name__ == "__main__":
    ...
    #     K=1, distance_metric=ClusteringDistance.CENTROID)
    # pca_plot_hierarchical(movies_df, hierarchical,
