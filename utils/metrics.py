...
class Metrics:
    ...
    def k_fold_cross_validation_eval(x, y, model, k: int, X_features: list = None, y_features: list = None, classes: list = None):
        ...
        errors = []
        k_metrics_per_class = {
            'accuracy': {label: [] for label in classes},
            'precision': {label: [] for label in classes},
            'recall': {label: [] for label in classes},
            'f1-score': {label: [] for label in classes},
            'tp-rate': {label: [] for label in classes},
            'fp-rate': {label: [] for label in classes},
        }
        ...
        for fold in folds:
            ...
            for metric_label in k_metrics_per_class:
                for label in classes:
                    k_metrics_per_class[metric_label][label].append(
                        metrics_df[metric_label][label])
        ...
        average_metrics = {metric_label: {label: np.mean(
            k_metrics_per_class[metric_label][label]) for label in classes} for metric_label in k_metrics_per_class}
        std_metrics = {metric_label: {label: np.std(
            k_metrics_per_class[metric_label][label]) for label in classes} for metric_label in k_metrics_per_class}
    ...
    def n_k_fold_cross_validation_eval(X, y, n, model, k: int, X_features: list = None, y_features: list = None, classes: list = None):
        ...
        for i in range(n):
            ...
            for metric_label in k_metrics_per_class:
                for label in classes:
                    k_metrics_per_class[metric_label][label].append(
                        n_average_metrics[metric_label][label])
                    avg_std_metrics[metric_label][label].append(
                        n_std_metrics[metric_label][label])

        # Compute the average and standard deviation metrics
        average_metrics = {metric_label: {label: np.mean(
            k_metrics_per_class[metric_label][label]) for label in classes} for metric_label in k_metrics_per_class}
        std_metrics = {metric_label: {label: np.mean(
            avg_std_metrics[metric_label][label]) for label in classes} for metric_label in k_metrics_per_class}
    ...
    def plot_metrics_heatmap_std(avg_metrics: dict, std_metrics: dict, plot_title=""):
        ...
        error_values = []
        for row in rows:
            error_values.append([std_metrics[column][row]
        ...
        for i in range(len(rows)):
            labels.append(
                [f"{values[i][j]:.3f} ± {error_values[i][j]:.3f}" for j in range(len(columns))])
        ...
        # plot the heatmap with the labels
        fig, ax = plt.subplots(figsize=(10, 10))
        ax = sns.heatmap(values, annot=labels, fmt='', cmap='RdYlGn',
                         xticklabels=xticks, yticklabels=yticks, vmin=0, vmax=1)
        ax.set_title(plot_title)
        plt.show()
    ...
    def plot_confusion_matrix_heatmap(cf_matrix, predicted_title="Predicted label", actual_title="Truth label", plot_title=""):
        ...
        for i in range(len(cf_matrix.index)):
            labels.append([f"{cf_matrix.iloc[i, j]}\n({cf_matrix_perc.iloc[i, j]:.2f}%)" for j in range(
                len(cf_matrix.columns))])
