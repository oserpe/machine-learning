...
class Kohonen(BaseEstimator):
    def __init__(self, max_iter, random_state, K, initial_radius, initial_lr,
    ...
    def fit(self, X):
        ...
        while iter < self.max_iter:
            radius = math.floor(self.radius_fn(
                iter, self.max_iter, self.initial_radius))
                ...
                for i in range(winner[0] - radius, winner[0] + radius + 1):
                    for j in range(winner[1] - radius, winner[1] + radius + 1):
                        if i >= 0 and i < self.K and j >= 0 and j < self.K:
                            self.w[i][j] += self.lr_fn(iter, self.max_iter,
                                                       self.initial_lr) * (x_i - self.w[i][j])
